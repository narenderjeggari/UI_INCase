...
export default function IdentifyProofingQuestions() {
      ...
      case "spiNumber":
        return (
          <Typography
            ...
            onClick={() => {
              fetchParamDetails(row.spiId, false);
            }}
          >
            {value && (
              <div
    ...
    try {
      const response =
        process.env.REACT_APP_ENV === "mockserver"
          ? await client.get(`${otherConfigInvesticaseIdentifyProofingQuestionsSearchURL}`)
          : await client.post(`${otherConfigInvesticaseIdentifyProofingQuestionsSearchURL}`, payload);

      setData(response.spideringIdentifiersList || []);
  ...
  const showDeleteParamDialog = (event, spiId) => {
    setSelectedParamIdForDelete(spiId);
  ...
  return (
    <>
                                ...
                                <Tooltip title="Edit" placement="left">
                                  <IconButton disabled={!isUpdateAccessExist()}>
                                    <EditNoteIcon
                                      ...
                                      onClick={(event) => {
                                        fetchParamDetails(row.spiId, true);
                                        event.preventDefault();
                                        event.stopPropagation();
                                      }}
                                    />
                              ...
                              {row.deleteFlag === true ? (
                                <Tooltip title="Delete" placement="right">
                                  <IconButton disabled={!isUpdateAccessExist()}>
                                    <DeleteForeverIcon
                                      onClick={(event) => {
                                        showDeleteParamDialog(event, row.spiId);
                                      }}
                                      sx={{ cursor: "pointer" }}
                                      fontSize="medium"
                                      {...(isUpdateAccessExist() && {
      ...
      <CustomModal
        ...
        title="View Identify Proofing Questions"
        maxWidth="md"
      >
        <DialogContent>
          <Stack mt={3}>
      ...
      <CustomModal
        ...
        aria-labelledby="modal-modal-title"
        aria-describedby="modal-modal-description"
        title="Edit Identify Proofing Questions"
        maxWidth="lg"
      >
